    ...
    def read(self,stack) -> None:
        ...
        while True:
            if len(stack) != 0:
                ...
                faces, boxes, scores, landmarks = self.api.detector.detect_align(img_new)
                #faces, boxes, scores, landmarks = self.detector.detect_align(img_new)
                if len(faces.shape) > 1:
                    results, score = self.api.face_rec.infer(faces, self.targets)
                    for idx, bbox in enumerate(boxes):
                        special_draw(img_new, bbox, landmarks[idx], self.names[results[idx] + 1], score[idx])
                        res = {"name":self.names[results[idx]+1],"score":score[idx]}
                        print("name={},score={}".format(res["name"],res["score"]))
